        ...
        namespace Detail
        {
            namespace Registry
            {
                ...
                public:
                    ...
                    static typename T::Result Do(TPrm && ... prm)
                    {
                        bool next = false;
                        return Next<T, 1>(next, std::move(std::make_tuple(std::forward<TPrm>(prm) ... )));
                    }
                ...
                private:
                    ...
                    {
                        return T::template Do<TPS>(std::get<Indexes>(prm) ... );
                    }

                    template <typename T, std::size_t I, typename TPrm>
                    static typename T::Result Next(bool &next,
                            typename Item<I>::Index::value_type,
                            TPrm && prm)
                    {
                    ...
                    static typename std::enable_if<I != Common::Detail::FakeHierarchyLength::value, typename T::Result>::type
                    Next(bool &next, ...)
                    {
                        next = false;
                        return {};
                    ...
                    template <typename T, std::size_t I, typename TPrm>
                    static typename std::enable_if<I != Common::Detail::FakeHierarchyLength::value, typename T::Result>::type
                    Next(bool &next, TPrm && prm)
                    {
                        auto res = Next<T, I>(next, std::size_t{}, std::forward<TPrm>(prm));
                        if (next)
                            res = Next<T, I + 1>(next, std::forward<TPrm>(prm));
                        return res;
                    }
                };
